    ...
    /// Tolerance for analytic gradient comparisons (tape vs hand-written).
    /// atol=1e-4: auto-pass gradients below f32 FD detection limit (~5e-4).
    /// rtol=1e-2: 1% relative tolerance accommodates f32 replay/accumulation ordering.
    const ANALYTIC_GRAD_RTOL: f32 = 1e-2;
    const ANALYTIC_GRAD_ATOL: f32 = 1e-4;
    ...
    #[test]
    fn test_titans_mlp_cms_gradient_l1_w_k_mem() {
        let cfg = titans_mlp_grad_check_config();
        let params = titans_mlp_params_for_grad_check(&cfg, 42);
        let (input_ids, target_ids) = titans_cms_make_test_data(&cfg);
        let pulse = both_active_pulse(cfg.k);
        ...
        let (checked, passed, max_err) = cms_check_weight_gradient(
            &params, &cfg, &input_ids, &target_ids, &grads, "titans_mlp_l1_w_k_mem", &pulse,
            |p| p.levels[1].w_k_mem.master(), |p, i, v| p.levels[1].w_k_mem.set(i, v), |g| g.levels[1].w_k_mem.master(),
            20, FD_EPS, FD_TOL,
        );
        eprintln!("titans MLP l1_w_k_mem: {passed}/{checked} pass, max_rel_err={max_err:.4e}");
        assert!(passed == checked, "titans MLP l1_w_k_mem: {passed}/{checked} passed");
    }

    #[test]
    fn test_titans_mlp_cms_gradient_l1_w_v_mem() {
        let cfg = titans_mlp_grad_check_config();
        let params = titans_mlp_params_for_grad_check(&cfg, 42);
        let (input_ids, target_ids) = titans_cms_make_test_data(&cfg);
        let pulse = both_active_pulse(cfg.k);
        ...
        let (checked, passed, max_err) = cms_check_weight_gradient(
            &params, &cfg, &input_ids, &target_ids, &grads, "titans_mlp_l1_w_v_mem", &pulse,
            |p| p.levels[1].w_v_mem.master(), |p, i, v| p.levels[1].w_v_mem.set(i, v), |g| g.levels[1].w_v_mem.master(),
            20, FD_EPS, FD_TOL,
        );
        eprintln!("titans MLP l1_w_v_mem: {passed}/{checked} pass, max_rel_err={max_err:.4e}");
        assert!(passed == checked, "titans MLP l1_w_v_mem: {passed}/{checked} passed");
    }

    #[test]
    fn test_titans_mlp_cms_gradient_l1_w_q_mem() {
        let cfg = titans_mlp_grad_check_config();
        let params = titans_mlp_params_for_grad_check(&cfg, 42);
        let (input_ids, target_ids) = titans_cms_make_test_data(&cfg);
        let pulse = both_active_pulse(cfg.k);
        ...
        let (checked, passed, max_err) = cms_check_weight_gradient(
            &params, &cfg, &input_ids, &target_ids, &grads, "titans_mlp_l1_w_q_mem", &pulse,
            |p| p.levels[1].w_q_mem.master(), |p, i, v| p.levels[1].w_q_mem.set(i, v), |g| g.levels[1].w_q_mem.master(),
            20, FD_EPS, FD_TOL,
        );
        eprintln!("titans MLP l1_w_q_mem: {passed}/{checked} pass, max_rel_err={max_err:.4e}");
        assert!(passed == checked, "titans MLP l1_w_q_mem: {passed}/{checked} passed");
    }

    #[test]
    fn test_titans_mlp_cms_gradient_l1_w_alpha() {
        let cfg = titans_mlp_grad_check_config();
        let params = titans_mlp_params_for_grad_check(&cfg, 42);
        let (input_ids, target_ids) = titans_cms_make_test_data(&cfg);
        let pulse = both_active_pulse(cfg.k);
        ...
        let (checked, passed, max_err) = cms_check_weight_gradient(
            &params, &cfg, &input_ids, &target_ids, &grads, "titans_mlp_l1_w_alpha", &pulse,
            |p| &p.levels[1].w_alpha, |p, i, v| p.levels[1].w_alpha[i] = v, |g| &g.levels[1].w_alpha,
            16, FD_EPS, FD_TOL,
        );
        eprintln!("titans MLP l1_w_alpha: {passed}/{checked} pass, max_rel_err={max_err:.4e}");
        assert!(passed == checked, "titans MLP l1_w_alpha: {passed}/{checked} passed");
    }
    ...
    fn class3_tape_vs_handwritten(cfg: &MAGConfig, label: &str) {
        ...
        let rtol = ANALYTIC_GRAD_RTOL;
        let atol = ANALYTIC_GRAD_ATOL;
