...
__global__ void delta_backward_segment_kernel(
    ...
    const float* __restrict__ alpha,      // [batch_size, seq_len]
    const float* __restrict__ theta,      // [batch_size, seq_len]
    const float* __restrict__ m_states,   // [batch_size, (seg_len+1)*d*d] — segment-local
    const float* __restrict__ d_y,        // [batch_size, seq_len, d]
    const float* __restrict__ d_m_seed,   // [batch_size, d*d] — seed from subsequent segment
    float* __restrict__ d_k_mem,          // [batch_size, seq_len, d] — full sequence, absolute offsets
    float* __restrict__ d_v_mem,
    float* __restrict__ d_q_mem,
    float* __restrict__ d_alpha,
    float* __restrict__ d_theta,
    float* __restrict__ d_m_out,          // [batch_size, d*d] — d_M to propagate to earlier segment
    ...
    float error_clip)
{
    ...
    delta_backward_segment_kernel<<<grid, block, smem_bytes>>>(
        ...
        d_M_work, t_start, t_end, d, seq_len, error_clip);
